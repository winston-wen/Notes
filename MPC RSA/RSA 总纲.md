...
(3) 符合 FIPS 186-4 标准. 重点是保证以下约束, 以防止 费马分解攻击 和 连分数攻击.
$$
\log_2(q-p)>\log_2\sqrt n-100.
$$
同时, 也不要让 $$\log_2 p$$ 与 $$\log_2 q$$ 相差太远, 一两个比特还是可以接受的. 否则短的那个质因子将成为弱点, 面临 Pollard p-1 攻击, 椭圆曲线分解 (ECM) 攻击, 甚至是试除法攻击等.
